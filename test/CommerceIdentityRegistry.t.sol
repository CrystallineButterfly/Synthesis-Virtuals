...
contract CommerceIdentityRegistryTest is Test {
    CommerceIdentityRegistry private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_virtuals");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new CommerceIdentityRegistry(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        CommerceIdentityRegistry freshController = new CommerceIdentityRegistry(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
